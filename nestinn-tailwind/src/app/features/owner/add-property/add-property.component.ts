...
export class AddPropertyComponent implements OnInit {
  ...
  pendingImgs: Record<number, File> = {};
    ...
    obs.subscribe({
      next: (r: any) => {
        ...
        if (!this.editId) {
          ...
          const hasPending = Object.keys(this.pendingImgs).length > 0;
          if (hasPending) {
            this.toast.success('Property created! Uploading images...');
            Object.entries(this.pendingImgs).forEach(([order, file]) => {
              this.doUpload(file, +order);
            });
            this.pendingImgs = {};
          } else {
  ...
  uploadImg(event: any, order: number) {
    ...
    if (!file) return;
    if (!this.createdId) {
      this.pendingImgs[order] = file;
      const reader = new FileReader();
      reader.onload = (e: any) => { this.uploadedImgs[order] = e.target.result; };
  ...
  doUpload(file: File, order: number) {
